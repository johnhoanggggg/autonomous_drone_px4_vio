# Autonomous Drone PX4 VIO — Handoff Archive (chronological log)

> This is the append-only session log, archived 2026-07-09. It is kept for forensics
> (why decisions were made, past debugging trails). For current project state, bring-up,
> params, and known gotchas, see `HANDOFF.md` — that is the authoritative starting point.
> Some guidance below is superseded (e.g. "keep EKF2_EV_CTRL=3"); trust HANDOFF.md, not this.
...
## Latest Update - 2026-07-07 Autonomous Hover (Offboard) prepared — NOT yet flown

Goal: short autonomous hover — climb to 0.30 m, hold 10 s, land — over the existing uXRCE-DDS link.

### New node: `offboard_hover` (px4_vio_bridge)
File: `ros2_ws/src/px4_vio_bridge/px4_vio_bridge/offboard_hover.py`. Registered console_script `offboard_hover`; built with `colcon build --packages-select px4_vio_bridge --symlink-install`.
- State machine `WAIT_POS -> STREAM -> ENGAGE -> CLIMB_HOLD -> LAND -> DONE`.
- Publishes `/fmu/in/offboard_control_mode` (position) + `/fmu/in/trajectory_setpoint` at 50 Hz (BEST_EFFORT/VOLATILE QoS), drives `/fmu/in/vehicle_command` for DO_SET_MODE(offboard)/ARM/NAV_LAND.
- Subscribes `/fmu/out/vehicle_local_position_v1` and `/fmu/out/vehicle_status_v1` (BEST_EFFORT/TRANSIENT_LOCAL) to latch hold x/y/yaw and confirm `arming_state==2`/`nav_state==14`.
- Verified all px4_msgs field names/constants exist (DO_SET_MODE=176, ARM_DISARM=400, NAV_LAND=21, ARMED=2, OFFBOARD=14).
- Params: `hover_height=0.30`, `hold_time=10.0`, `rate_hz=50`, `stream_time=1.0`, `engage_timeout=5.0`, `climb_timeout=8.0`, `reach_tol=0.07`, `max_flight_time=40.0`, `auto_arm=false`.

### Safety design (why this is the shape it is)
- **`auto_arm` defaults FALSE** — runs the whole flow but never arms, so the props-off dry run confirms setpoint streaming + OFFBOARD request without flight. Live flight needs `auto_arm:=true`.
- Aborts (never arms) if OFFBOARD+ARM not confirmed within `engage_timeout`.
- `max_flight_time` watchdog forces LAND; lost local position in flight -> LAND; Ctrl-C while armed -> AUTO.LAND (never mid-air disarm). Ends the flight with `NAV_LAND` (auto-disarm on ground detect).

### Decisions captured this session
- RC transmitter IS bound (manual override / kill available) — keep `COM_RC_IN_MODE=0`.
- Airframe has flown before and is tuned, so autonomous hover is reasonable.
- Relevant params read: `COM_ARM_WO_GPS=1` (arm w/o GPS OK), `EKF2_HGT_REF=3`, `COM_RCL_EXCEPT=0`, `COM_OBL_RC_ACT=0` (offboard-loss -> Position), `NAV_RCL_ACT=1`.
...
```
Live flight (props on, RC ready, area clear): same command with `-p auto_arm:=true`.

### Status / next step
Node built and reviewed. See the 2026-07-09 update below for the dry-run result and fixes.

## Latest Update - 2026-07-09 Dry run PASSED + link/packaging findings

### Props-off dry run PASSED
`ros2 run px4_vio_bridge offboard_hover --ros-args -p auto_arm:=false -p hover_height:=0.30 -p hold_time:=10.0` walked the full sequence with **no arming**: `latched hold -> STREAM -> ENGAGE -> CLIMB_HOLD -> LAND -> DONE`. Key result: **PX4 accepted OFFBOARD while disarmed** (`flag_control_offboard_enabled` went true in <0.1 s), so the earlier "offboard only reported when armed" worry did NOT materialize on this build. Validated: VIO position latch, 50 Hz setpoint streaming, `DO_SET_MODE->OFFBOARD` command, and control-mode confirmation. Only the actual arm+climb is unexercised. Motors never commanded.

### Fix 1 — offboard node uses vehicle_control_mode, not vehicle_status
This PX4 build's `dds_topics.yaml` does **not** publish `vehicle_status`/`vehicle_status_v1` (probe got 0), but **does** publish `/fmu/out/vehicle_control_mode`. `offboard_hover.py` now confirms arm/offboard from `VehicleControlMode.flag_armed` / `flag_control_offboard_enabled`. If porting other offboard code, use vehicle_control_mode on this vehicle.

### Fix 2 — px4_vio_bridge is ament_cmake, not ament_python
Adding a node executable is NOT just a setup.py entry_point — those are **vestigial/ignored** here. The build is `CMakeLists.txt` (`ament_cmake` + `ament_python_install_package`), and console wrappers are installed by explicit name via `install(PROGRAMS scripts/<name> ... DESTINATION lib/${PROJECT_NAME})`. To add a node: (1) create `scripts/<name>` wrapper (`from px4_vio_bridge.<mod> import main`), chmod +x, (2) add it to the `install(PROGRAMS ...)` list in CMakeLists.txt, (3) rebuild. `offboard_hover` is now registered (`ros2 pkg executables px4_vio_bridge` lists it).

### TELEM2/DDS link is marginal (works USB-free but near the edge)
Full analysis is in README "Known Issues & Operational Notes". Summary: USB (`/dev/ttyACM0`) is dev/debug only; flight comms is TELEM2/uXRCE (`/dev/ttyAMA0`, 921600). A cold uXRCE session sometimes gets stuck re-establishing its whole datawriter graph (0 telemetry to ROS); it established after a USB reconnect and then stayed streaming at 50 Hz even with USB back out, and came up clean USB-free across several Pixhawk power-cycles. Fragile phase is **establishment**, not steady state -> marginal serial link (likely an unreliable TELEM2 cable; grounding margin helps at the edge). Before flight: replace with short shielded/twisted cable, consider `SER_TEL2_BAUD=460800`, and verify DDS stability on battery power USB-free. Without USB there is no NSH shell, so USB-free DDS recovery = physical Pixhawk power-cycle.

### VIO "shake to align" is normal VIO init
Documented in README. Yaw origin is arbitrary at boot + VIO needs motion to converge; EKF2 then `reset_yaw_to_vision`. Make a deliberate pre-arm move-to-initialize a checklist step; separate any repeatable residual offset (static mount) -> `vio_yaw_offset_deg`.

### Remaining before live flight
Pre-flight gate green (have it: uxrce connected, `xy_valid`/`z_valid`/`v_xy_valid`/`heading_good_for_control` true). Then props on, RC bound as kill, area clear, run with `-p auto_arm:=true`. Caveats still open: 0.30 m is very low with vision-only height (no rangefinder) — confirm Z steady near floor or raise to 0.5-0.6 m; and confirm TELEM2 link reliability on battery power first.
