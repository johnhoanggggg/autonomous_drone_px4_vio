...
parser.add_argument('--publish-image', type=parse_bool, default=True, help='Publish /rtabmap/image.')
...
class Ros2Node(dai.node.ThreadedHostNode):
    def __init__(self):
        ...
        self.img_pub = self.ros_node.create_publisher(Image, '/rtabmap/image', 10)
    ...
    def run(self):
        while self.mainLoop():
            ...
            imgFrame = self.inputImg.get() if self.publish_image else None
            ...
            if self.publish_image and imgFrame is not None:
                ...
                img_msg.height = imgFrame.getHeight()
                img_msg.width = imgFrame.getWidth()
                img_msg.encoding = 'mono8'
                img_msg.step = imgFrame.getWidth()
                img_msg.data = bytes(imgFrame.getData())
