"""Launch interactive Foxglove-click waypoint flight, with the flight bag.

Run the SLAM/PX4/Foxglove stack (`rtabmap_slam_px4.launch.py`) first; this
launch adds only the flight node and its recorder. The Foxglove bridge must be
started with client publishing enabled — see `foxglove_capabilities` and
`foxglove_client_topic_whitelist` in that launch file.
"""
...
def generate_launch_description():
    ...
    default_bag_output = str(bag_root / f"offboard_waypoint_{stamp}")

    arguments = [
        DeclareLaunchArgument("auto_arm", default_value="false"),
        DeclareLaunchArgument("hover_height", default_value="0.3"),
        # Interactive flight needs time to click; the hold-yaw default of 40 s
        # is a scripted-sequence budget. Watch the battery - hover draw is
        # ~269 W, so keep sessions short and land on your own terms.
        ...
        DeclareLaunchArgument("accept_waypoint_yaw", default_value="false"),
    ...
    node = Node(
        ...
        executable="offboard_waypoint",
        name="offboard_waypoint",
        output="screen",
        emulate_tty=True,
        parameters=[
            {
                ...
                "accept_waypoint_yaw": typed("accept_waypoint_yaw", bool),
    ...
    stop_after_flight = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[
                EmitEvent(
                    event=Shutdown(
                        reason="offboard_waypoint finished; finalizing flight bag"
                    )
                )
            ],
        )
